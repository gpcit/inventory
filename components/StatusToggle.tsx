...
export default function StatusToggle({ loading, onChange} : {  loading: boolean, onChange: () => void}){
    ...
    const handleToggleChange = () => {
            ...
            setToggle(toggle === 'active' ? 'inactive' : 'active')
            onChange();
    ...
    <div className="flex items-center flex-col-reverse me-5">
        ...
        <label
                ...
                className={`cursor-pointer relative w-14 h-6 shadow-md shadow-black rounded-full p-1 transition-colors duration-200 ease-in-out ${toggle === 'active' ? 'bg-green-500' : 'bg-red-500'}`}
            >
                <span
