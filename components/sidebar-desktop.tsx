            ...
            {props.sidebarItems.links.map((link, index) => (
              <Link key={index} href={link.href}>
                <SidebarButton
                  ...
                  icon={link.icon}
                  className='focus:ring-2 border-hidden hover:text-green-400 focus:ring-white px-5 w-60 border border-green-500 my-4 py-8  shadow-xl hover:shadow-green-500/45 shadow-gray-800'
                >
                  {link.label}
                </SidebarButton>
              </Link>
