          ...
          let values = []
            if(searchQuery) {
              ...
              data = `SELECT * FROM ${tableName}
                      WHERE (assigned_to LIKE ? OR department LIKE ? OR email_password LIKE ? OR imei LIKE ? OR number LIKE ? OR serial_number LIKE ?) 
                      AND is_active_id = 1 
                      LIMIT ? 
                      OFFSET ?`;
              values = [`%${searchQuery}%`, `%${searchQuery}%`, `%${searchQuery}%`, `%${searchQuery}%`, `%${searchQuery}%`, `%${searchQuery}%`, itemPerPage, (page - 1) * itemPerPage]
